...
1. Run the precondition script:
   ...
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-precondition.js --kind knowledge
   ```
...
1. Run the archetypes CLI to fetch available archetypes from the server:
   ...
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-archetypes.js --kind knowledge
   ```
...
2. If creating a new L0 board:
   ...
   - If config has no `boardSlug`, make the **connect-now offer** — ask with **AskUserQuestion** "Connect to ProvenMap now?" (**Connect now** / **Not now**):
     - **Connect now** → run the browser login here, printing each JSON `display` verbatim **in your reply** (the Bash output panel is collapsed for the user): `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --start`, then `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --poll --analyze-cmd analyze-docs` (generous Bash timeout, e.g. 250s). On `status: "complete"`, continue; anything else — stop, the display explains.
