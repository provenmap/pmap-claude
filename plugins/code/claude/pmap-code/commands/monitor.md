...
- **Connect now** → run the browser login here, printing each `display` verbatim in your reply: `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --start`, then `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --poll --analyze-cmd analyze` (Bash timeout ≥250s). On `status: "complete"` resume the failed step; anything else — stop, the display explains.
