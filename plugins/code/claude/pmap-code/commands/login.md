...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --start          # add --rebind only for /login switch
# add --base-url <url> when $ARGUMENTS contains an http(s) URL
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --poll --analyze-cmd analyze
```
