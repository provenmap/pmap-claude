...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-insights.js --list-insight-skills --board-slug <boardSlug> --domain code
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-insights.js --build-context --board-slug <board> --demo --out .provenmap/insights/<board>.context.json --summary --domain code
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-insights.js --save-insight /tmp/insight-<slug>.json --board-slug <board> --demo --require-pack --push --host claude --domain code
```
...
**Close:** `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-status.js --after demo-insights --domain code` — print verbatim.
