...
Prov Code runs in **Claude Code**. Its sibling, **Prov Work** (`prov-work`), does the same for a document set → a knowledge board. Both emit the same node/edge wire model and sync through the same engine.
...
/plugin install prov-code@provenmap
```
