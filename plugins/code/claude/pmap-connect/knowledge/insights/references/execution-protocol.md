...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-insights.js --build-context --board-slug <boardSlug> --out .provenmap/discover/packs/<boardSlug>.context.json --summary --domain connect
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-insights.js --save-insight /tmp/insight-<slug>.json --board-slug <pack.primaryBoardDefault> --require-pack --push --host claude --domain connect
```
