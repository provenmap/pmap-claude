...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --start --host claude --domain connect --plugin-version 0.17.0   # add --rebind only for /login switch
# add --base-url <url> when the user's argument is an http(s) URL
```
...
**Close:** `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-status.js --after login --domain connect` — print verbatim.
