...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-login.js --start --host claude --domain connect --plugin-version 0.19.0   # add --rebind only for /login switch
# add --base-url <url> when the user's argument is an http(s) URL
```
...
**Outcome:** `node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-status.js --brief --domain connect --command login` → Done · Left · Next, per `${CLAUDE_PLUGIN_ROOT}/knowledge/outcome/SKILL.md`.
