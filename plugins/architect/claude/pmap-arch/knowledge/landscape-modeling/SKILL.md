...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-arch.js --scan-repos --paths <p1,p2,...>
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-arch.js --validate diagram --file <payload.json> [--known-slugs <existing,slugs>]
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/pmap-arch.js --render-scaffold --file <scaffold.json>
```
